...
import '../api.dart';
import '../models/announcement_store.dart';
import '../theme.dart';
...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
    ...
    return Row(children: [
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(
          ...
          style: const TextStyle(fontSize: 23, fontWeight: FontWeight.w900, letterSpacing: -0.3, color: AppColors.text, height: 1.2),
        ),
        const SizedBox(height: 7),
        Row(children: [
          const Icon(Icons.bolt_rounded, size: 14, color: AppColors.nu2),
      ...
      // أفاتار المستخدم (حقيقي حسب الجنس) + شارة الاشتراك — ضغطة بتفتح تاب حسابي.
      GestureDetector(
        onTap: () => widget.onGo(3),
        child: Column(children: [
          GenderAvatar(
            ...
            size: 52,
            ringColor: pro ? AppColors.nu : AppColors.line,
          ),
          const SizedBox(height: 6),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
            decoration: BoxDecoration(
              color: pro ? AppColors.nu.withValues(alpha: .16) : AppColors.card,
              borderRadius: BorderRadius.circular(999),
              border: Border.all(color: pro ? AppColors.nu.withValues(alpha: .4) : AppColors.lineSoft),
            ),
        ...
        child: Container(
          ...
          child: const Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              SizedBox(height: 4),
              Text('4 خطوات قصيرة تجمع بيانات التدريب والتغذية مرة واحد������',
  ...
  Widget _trialBanner() {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: AppColors.good.withValues(alpha: .10),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: AppColors.good.withValues(alpha: .35)),
        ),
        child: Row(children: [
  ...
  Widget _coachCard() {
    ...
    return Container(
      padding: const EdgeInsets.all(EFS.xl),
      decoration: EFDeco.card(),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const EFEyebrow('ملخص النهاردة', color: AppColors.nu),
        ...
        Row(children: [
          ...
          Expanded(
              child: _stat(
                  ...
                  delta == null ? AppColors.muted : AppColors.warn)),
        ]),
        const SizedBox(height: 16),
        Container(
          width: double.infinity,
          ...
          child: Text(_coachLine(),
              style: const TextStyle(fontSize: 12.5, height: 1.6, color: AppColors.text)),
        ),
      ]),
    );
  }

  Widget _statDivider() => Container(width: 1, height: 30, color: AppColors.lineSoft);
  ...
  Widget _stat(String label, String value, Color color) => Column(
        ...
        children: [
          ...
          Text(value, textAlign: TextAlign.center,
              style: TextStyle(
                  color: color, fontWeight: FontWeight.w900, fontSize: 13.5)),
        ],
      );

  // الكارت الرئيسي للإجراء (تمرين اليوم / جلسة جارية / راحة...).
  // التغيير بصري بالكامل — نفس الـ params والـ callbacks. النص على الأزرار
  // الملونة أصبح داكنًا (contrast واضح) بدل الأبيض الضعيف على التركواز/البرتقالي.
  Widget _actionCard({required Color color, required IconData icon, required String eyebrow,
      ...
      Container(
        padding: const EdgeInsets.all(EFS.xl),
        decoration: EFDeco.accentCard(color),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(
              width: 48, height: 48,
          ...
          Text(subtitle, style: const TextStyle(color: AppColors.muted, fontSize: 13, height: 1.5)),
          const SizedBox(height: 16),
          if (secondaryButton == null)
            SizedBox(width: double.infinity, height: 50, child: FilledButton(
              onPressed: onTap,
              style: FilledButton.styleFrom(backgroundColor: color, foregroundColor: const Color(0xFF0A0F14)),
              child: Text(button),
            ))
          else
            Row(children: [
              Expanded(child: SizedBox(height: 50, child: FilledButton(
                onPressed: onTap,
                style: FilledButton.styleFrom(backgroundColor: color, foregroundColor: const Color(0xFF0A0F14)),
                child: Text(button),
              ))),
              ...
              SizedBox(height: 50, child: OutlinedButton(
                ...
                style: OutlinedButton.styleFrom(
                  ...
                  side: BorderSide(color: color.withValues(alpha: .6), width: 1.4),
                ),
                child: Text(secondaryButton, style: const TextStyle(fontSize: 13)),
              )),
            ]),
        ]),
      );
  ...
  Widget _nutritionCard() {
    ...
    return _smallCard(
      ...
      progress: target > 0 ? (calories / target).toDouble() : null,
      onTap: () => widget.onGo(2),
    );
  }
  ...
  Widget _waterCard() {
    ...
    return _smallCard(
      ...
      progress: (water / 2500).toDouble(),
      onTap: () => widget.onGo(2),
    );
  }

  // كارت صغير (تغذية/مياه) — ضغطة بنبضة + شريط تقدم حقيقي اختياري تحت القيمة.
  Widget _smallCard({required Color color, required IconData icon, required String title,
      required String value, required String subtitle, required VoidCallback onTap, double? progress}) =>
      Pressable(
        onTap: onTap,
        child: Container(
          padding: const EdgeInsets.all(EFS.l),
          decoration: EFDeco.card(),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Container(
                width: 34, height: 34,
                decoration: BoxDecoration(color: color.withValues(alpha: .15), borderRadius: BorderRadius.circular(11)),
                child: Icon(icon, color: color, size: 19),
              ),
              const SizedBox(width: 9),
              Text(title, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 12.5, color: AppColors.text)),
            ]),
            const SizedBox(height: 14),
            Text(value, maxLines: 1, overflow: TextOverflow.ellipsis,
                style: const TextStyle(fontSize: 17.5, fontWeight: FontWeight.w900, color: AppColors.text)),
  ...
  Widget _weeklyProgress() {
    ...
    // (منطق العدّ محفوظ بالظبط — أضفنا بس تفكيك الأيام لعرضها كنقاط بدل شريط.)
    final seenDays = <String>{};
    final doneWeekdays = <int>{};
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            width: 34, height: 34,
            decoration: BoxDecoration(color: AppColors.wo.withValues(alpha: .15), borderRadius: BorderRadius.circular(11)),
            child: const Icon(Icons.insights_rounded, color: AppColors.wo2, size: 19),
          ),
          const SizedBox(width: 10),
          const Expanded(child: Text('تقدم التدريب هذا الأسبوع', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14.5, color: AppColors.text))),
        ...
        Row(
          ...
          children: [
            for (var i = 0; i < 7; i++)
              _dayDot(dayLabels[i], doneWeekdays.contains(i), i == todayIdx),
          ],
        ),
      ]),
    );
  ...
  // نقطة يوم في الأسبوع: ممتلئة لو اتمرّن، محاطة لو هو النهاردة.
  Widget _dayDot(String label, bool done, bool isToday) {
    final Color bg = done ? AppColors.wo : (isToday ? AppColors.wo.withValues(alpha: .14) : AppColors.card2);
    final Color lbl = isToday ? AppColors.wo2 : AppColors.faint;
    return Column(children: [
      Container(
        ...
        decoration: BoxDecoration(
          ...
          border: (isToday && !done) ? Border.all(color: AppColors.wo.withValues(alpha: .6), width: 1.4) : null,
        ),
        child: done ? const Icon(Icons.check_rounded, color: Color(0xFF2A1105), size: 17) : null,
      ),
      const SizedBox(height: 5),
      Text(label, style: TextStyle(color: lbl, fontSize: 10, fontWeight: isToday ? FontWeight.w900 : FontWeight.w600)),
    ]);
  }
    ...
    return Pressable(
      ...
      child: Container(
        ...
        child: Row(children: [
          Container(
            width: 40, height: 40,
            decoration: BoxDecoration(color: AppColors.nu.withValues(alpha: .13), borderRadius: BorderRadius.circular(13)),
            child: const Icon(Icons.monitor_weight_outlined, color: AppColors.nu, size: 22),
          ),
          const SizedBox(width: 12),
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('الوزن والمتابعة', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14.5, color: AppColors.text)),
            SizedBox(height: 3),
