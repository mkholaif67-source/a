...
function bounds(entry, meal) {
  ...
  if (isAddedFat(entry)) return { min: 2, max: 10, step: 1, optional: true };
  ...
  if (/طحينة|طحينه/.test(n)) return { min: 20, max: 40, step: 5 };
  if (/زبادي/.test(n)) return { min: 60, max: 200, step: 5 };
  if (/بيض/.test(n) && !/جبنة بيضاء/.test(n)) return { min: 50, max: 150, step: 5 };
  if (/فول/.test(n) && !/سوداني/.test(n)) return { min: 80, max: 200, step: 5 };
  if (/تونة|تونه/.test(n)) return { min: 100, max: 150, step: 5 };
...
function enforceDailyCaps(plan) {
  let oil = 0, bread = 0, yogurt = 0, tuna = 0;
  for (const meal of plan.meals || []) {
    for (const f of meal.foods || []) {
      const n = nameOf(f);
      if (isAddedFat(f)) {
        const room = Math.max(0, 20 - oil); setGrams(f, Math.min(Number(f.grams) || 0, room)); oil += Number(f.grams) || 0;
      }
      ...
      if (/تونة|تونه/.test(n)) {
        const room = Math.max(0, 200 - tuna);
