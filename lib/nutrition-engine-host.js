...
function _efSizeAddedFats(plan){
  ...
  for(var m=0;m<meals.length;m++){
    ...
    for(var i=0;i<fs.length;i++){
      var f=fs[i];
      if(_efIsAddedFat(f)) added.push({meal:meals[m],f:f});
  ...
  for(var m=0;m<meals.length;m++){
    ...
    for(var i=0;i<meal.foods.length;i++){
      ...
      if(_efIsAddedFat(f)){
        ...
        _efSetGramsFine(f,Math.max(2,Math.min(10,Math.round(g))));
      } else {
...
function _efTrimDayToTolerance(plan){
  ...
  while(_efDayCalories(plan)>tgt+100 && guard++<120){
    var candidates=[];
    for(var m=0;m<meals.length;m++){
      var meal=meals[m]; if(!meal||meal._autoPreWorkout)continue;
      var fs=meal.foods||[];
...
function _efAddDynamicFatsForCalories(plan){
  ...
  var remaining=Math.max(0,20-used);if(remaining<2)return;
  ...
  for(var r=0;r<real.length&&_efDayCalories(plan)<tgt-100&&remaining>=2;r++){
    ...
    var grams=Math.min(10,remaining,Math.floor(Math.min(need,room)/8.84));
    if(grams<2)continue;
    var existing=(meal.foods||[]).find(function(f){return _efIsAddedFat(f);});
    if(existing){
      var cur=Number(existing.grams)||0, add=Math.min(10-cur,grams);
